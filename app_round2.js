...
const KEY_LAST = 'rp_last_v22';
const KEY_HIST = 'rp_hist_v22';
...
function renderMemorize(){
  ...
  const rd = state.round===2 ? '②' : '①';
  $('memoInfo').textContent = `ラウンド:${rd} / ペア数:${state.pairCount} / 記憶:${sec}秒 / 難易度:${d} / 抽象:${m}（語彙規模: 約 ${TOTAL_VOCAB_SIZE.toLocaleString()} ）`;
...
async function startFlow(fromResume=false){
  ...
  const time2El = $('time2');
  const seconds2=time2El ? Number(time2El.value) : seconds1;
  ...
  state = {
    ...
    round: 1,
    pairs: makePairs(pairCount, difficulty, absMix), // Round1で固定し、Round2も同一問題
...
function renderHistory(){
  ...
  h.forEach((x,idx)=>{
    box.insertAdjacentHTML('beforeend', `
      <div class="item">
        <div class="histRow">
          <div class="histScore">${(x.pct2 ?? x.pct ?? '')}% (${(x.correct2 ?? x.correct ?? '')}/${(x.total2 ?? x.total ?? '')})</div>
          <div class="muted small">${escapeHtml(x.when)}</div>
        </div>
        <div class="histMeta">
          <span>ペア:${x.pairCount}</span>
          <span>秒①:${x.seconds1 ?? x.seconds ?? ''}</span>
          <span>秒②:${x.seconds2 ?? ''}</span>
...
function applyStateToSetup(s){
  ...
  $('time').value=String(s.seconds1 ?? s.seconds ?? 30);
  const t2 = $('time2');
  if(t2) t2.value=String(s.seconds2 ?? s.seconds1 ?? s.seconds ?? 30);
...
function resume(){
  ...
  state=s;
  // backward compatibility (older sessions)
  if(state.seconds1==null && state.seconds!=null) state.seconds1 = state.seconds;
  if(state.seconds2==null) state.seconds2 = state.seconds1 ?? 30;
  if(state.round==null) state.round = 1;
  if(!state.answers1 && state.answers){ state.answers1 = state.answers; }
  if(!state.answers2){ state.answers2 = Array(state.pairCount||0).fill(''); }
  if(!state.score1) state.score1 = null;
  if(!state.score2) state.score2 = null;

  applyStateToSetup(s);
  if(s.phase==='memorize'){
    go('memorize'); renderMemorize();
    // 再開時はタイマーはリセット（簡潔に）
    const sec = (state.round===2 ? (state.seconds2 ?? state.seconds1 ?? state.seconds) : (state.seconds1 ?? state.seconds));
    startTimer(sec, ()=>{ state.phase='recall'; saveLast(); go('recall'); renderRecall(); });
...
$('check').onclick = ()=>{
  ...
  // スコアをラウンド別に保持
  const sc = score();
  if(state.round===2) state.score2 = sc; else state.score1 = sc;
  state.lastScore = sc;
  saveLast();
  go('result');
  renderResult();
...
$('retry').onclick = ()=>{
  ...
  $('time').value=String(state.seconds1 ?? state.seconds);
  const t2=$('time2');
  if(t2) t2.value=String(state.seconds2 ?? state.seconds1 ?? state.seconds);
...
$('finish').onclick = ()=>{
  // Round1 の結果画面では Round2 に進む（同一問題で再挑戦）
  ...
  // Round2（最終）の場合：履歴保存して終了
  const sc2 = state.score2 || state.lastScore || score();
  const sc1 = state.score1 || null;

  const h = loadHist();
  h.unshift({
    when: state.when,
    pairCount: state.pairCount,
    seconds1: state.seconds1 ?? state.seconds,
    seconds2: state.seconds2 ?? '',
    difficultyLabel: state.difficultyLabel,
    absMixLabel: state.absMixLabel,
    correct1: sc1 ? sc1.correct : null,
    total1: sc1 ? sc1.total : null,
    pct1: sc1 ? sc1.pct : null,
    correct2: sc2.correct,
    total2: sc2.total,
    pct2: sc2.pct
  });
  saveHist(h);
